...
Description: Single-well pump test to derive transmissivity from waterlevel measurements.
             ...
             1. Resample and/or slice waterlevel and airpressure timeseries (optional).
             2. Correct waterlevel with airpressure data
             3. Auto-detect drawdown events in waterlevel timeseries.
             4. Separate waterlevels for each event between drawdown and fill
             5. Interpolate waterlevels for each drawdown and fill separately (optional) 
             6. Compute Transmissivity (T) as follows:
                 T = 2.3 * Q / 4 * Pi * dsw
                 Q = pumping rate
                 dsw: change in drawdown per log-cycle
             7. Write events, waterlevels drawdown and waterlevels fill into csv files  
...
sr = "1min" # temporal resampling rate (e.g., 5min, H, D, M)
stat = 'mean' # temporal resampling statistics 
###
t_int = True # temporal interpolation 
func = 'cubic'  # temporal interpolation function for drawdown events 
...
if t_int is True:
    ...
    wL = wL.interpolate(method=func)
    aP = aP.interpolate(method=func)
...
plt.grid()
plt.tight_layout()
plt.savefig(wdir + 'pisc_events.png', dpi=300)
plt.show()
plt.close()
